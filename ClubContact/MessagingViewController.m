...
@interface MessagingViewController ()<TextCellDelegate,MediaCellDelegate, UIImagePickerControllerDelegate>
{
...
- (void)keyboardDidShow:(NSNotification *)notification
{
    ...
    int someotherOffSet = 102 + (self.msgInPutView.frame.size.height - 40.0f);
    ...
    [UIView animateWithDuration:0.3
                     ...
                     animations:^
     {
         if (isOffSet == true) {
             ...
             self.msgInPutView.frame=CGRectMake(0,self.view.frame.size.height-self.msgInPutView.frame.size.height,  self.msgInPutView.frame.size.width, self.msgInPutView.frame.size.height);
...
- (IBAction)sendMessageNow:(id)sender
{
    ...
    if (messageText.length == 0) {
        ...
        [[AppController sharedInstance] addMessageToPatient2:msgSession.pid messagetext:messageText msgimg:nil sessionID:msgSession.postid WithCompletion:^(BOOL success, NSString *message, NSMutableArray *conditions) {
            [SVProgressHUD dismiss];
            if (success)
            {
                 [repeatTimer invalidate];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
     ...
     [SVProgressHUD showWithStatus:@"Sending..."];
    [[AppController sharedInstance] addMessageToPatient2:msgSession.pid messagetext:@"" msgimg:selectedImage sessionID:msgSession.postid WithCompletion:^(BOOL success, NSString *message, NSMutableArray *conditions) {
      [SVProgressHUD dismiss];
            if (success)
            {
                 [repeatTimer invalidate];
