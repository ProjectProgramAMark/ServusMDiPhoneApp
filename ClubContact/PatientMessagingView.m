...
@interface PatientMessagingView ()<TextCellDelegate,MediaCellDelegate, UIImagePickerControllerDelegate>
{
...
@implementation PatientMessagingView

@synthesize patientID;
@synthesize delegate;
@synthesize msgSession;
@synthesize isClosedSessionOpen;
@synthesize doctorV2;
...
- (void)cancelPop {
    [[AppController sharedInstance] closeMSGSessionRec:doctorV2.uid sessionID:msgSession.postid completion:^(BOOL success, NSString *message)  {
...
- (void)loadMessages {
  //  [repeatTimer invalidate];
    ...
    [[AppController sharedInstance] getMessagesBySession2:doctorV2.uid sessionID:msgSession.postid WithCompletion:^(BOOL success, NSString *message, NSMutableArray *conditions)
     {
...
- (void)getMessageStatus {
    //[repeatTimer2 invalidate];
    [[AppController sharedInstance] getMSGSessionsByID2:doctorV2.uid sessionID:msgSession.postid WithCompletion:^(BOOL success, NSString *message, MSGSession *msgsession) {
        
        if (success) {
            
            if ([msgsession.mstatus intValue] == 3) {
                if (isClosedShow == false) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.messageImageView.contentMode = UIViewContentModeScaleAspectFit;
    cell.CustomDelegate=self;
    cell.timestampLabel.text = dateString;
    if ([message.sendertype isEqual:@"Patient"]) {
        [cell.AvatarImageView setImageWithURL:[NSURL URLWithString:message.fromprofilepic]];
...
-(void)textViewDidChange:(UITextView *)textView
{
    ...
    if (size.height > 27.0f) {
        ...
        self.msgInPutView.frame = CGRectMake(0, (self.view.frame.size.height - offSetKeyboard - 40)  -  size.height, self.msgInPutView.frame.size.width, 40 + size.height);
...
{
    if (self.messageField2.text.length > 140) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Error" message:@"You can only send maximum of 140 characters at once" delegate:self cancelButtonTitle:@"OK" otherButtonTitles: nil];
        [alert show];
        return;
    }
    ...
    if (messageText.length == 0) {
        ...
        [[AppController sharedInstance] addMessageToPatientRec:doctorV2.uid messagetext:messageText msgimg:nil sessionID:msgSession.postid WithCompletion:^(BOOL success, NSString *message, NSMutableArray *conditions) {
            [SVProgressHUD dismiss];
            if (success)
            {
                [repeatTimer invalidate];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    [SVProgressHUD showWithStatus:@"Sending..."];
    [[AppController sharedInstance] addMessageToPatientRec:doctorV2.uid messagetext:@"" msgimg:selectedImage sessionID:msgSession.postid WithCompletion:^(BOOL success, NSString *message, NSMutableArray *conditions) {
        [SVProgressHUD dismiss];
        if (success)
        {
            [repeatTimer invalidate];
