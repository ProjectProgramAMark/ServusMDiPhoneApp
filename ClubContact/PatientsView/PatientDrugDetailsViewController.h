//
//  PatientDrugDetailsViewController.h
//  ClubContact
//
...
@class PatientDrugDetailsViewController;             //define class, so protocol can see MyClass
@protocol PatientDrugDetailsDelegate <NSObject>   //define delegate protocol
- (void) refreshRefillMedication;  //define delegate method to be implemented within another class
@end

@interface PatientDrugDetailsViewController : UIViewController <UITableViewDataSource, UITableViewDelegate> {
...
@property (nonatomic) BOOL isSharedRecord;
...
@property (nonatomic, weak) id <PatientDrugDetailsDelegate> delegate;
