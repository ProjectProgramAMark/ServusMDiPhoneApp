//
//  SendPatientRecViewController.m
//  ClubContact
//
...
@interface SendPatientRecViewController ()

@end

@implementation SendPatientRecViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  /*  NSString *isOnline = @"Offline";
    ...
    cell.costLabel.text = [NSString stringWithFormat:@"%@ tokens", patientInfo2.ccost];
    cell.clipsToBounds = YES;
    ...
    if (isPasscodeUpdate == true) {
        ...
        if (buttonIndex == 1) {
            ...
            [[AppController sharedInstance] checkPMasterPasscode:passcodeString completion:^(BOOL success, NSString *message) {
                ...
                if (success) {
                    [SVProgressHUD showWithStatus:@"Sending..."];
                    [[AppController sharedInstance] createShareRecord:doctorSend.uid patientid:profPatient.postid WithCompletion:^(BOOL success, NSString *message, NSString *msgID) {
                        [SVProgressHUD dismiss];
                        if (success) {
                            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Sent" message:@"Patient records sent" delegate:self cancelButtonTitle:@"Ok" otherButtonTitles: nil];
                            [alert show];
